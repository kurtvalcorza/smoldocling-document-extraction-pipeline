...
|---|---|---|---|---|
| `smoldocling_document_extraction_colab.ipynb` (`E2E`) | `bc77841` / `aaf8fbf0` | 2026-09-21 | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-smoldocling-document-extraction` v3; image `torch 2.10.0+cu128` / `transformers 5.0.0` before the pinned install, `torch 2.14.0+cu130` / `transformers 4.57.6` after, Python 3.12.13, `cuda:0`, float32) | **PASSED** — 11/11 code cells ok (1 restart after install cell); 36 files, 560 MB staged from the Hub into a clean cache; comparison {cer: {empty: 1, constant: 0.944, frozen: 1.439, adapted: 0.782}, wer: {empty: 1, constant: 0.999, frozen: 2.491, adapted: 0.975}, cer_macro: {empty: 1, constant: 0.952, frozen: 1.392, adapted: 0.882}, exact_match: {empty: 0, constant: 0, frozen: 0, adapted: 0.014}, delta_vs_frozen: {cer: -0.657, wer: -1.516, cer_macro: -0.51, exact_match: 0.014}, hypothesis_length: {ref_chars: 6169, frozen_hyp_chars: 4839, adapted_hyp_chars: 5225, frozen_truncated: 29, adapted_truncated: 2}}; drawing / scene / page check frozen vs adapted {frozen: {word_error_rate: 0.192}, adapted: {word_error_rate: 0.709}}; reload parity {identical_lines: 8, of: 8}; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-smoldocling-document-extraction/v3/evidence/` in the workspace |
...
|---|---|---|---|---|---|
| 2026-09-21 | `7415a38` / `c27ef8ae` (pre-flight: the build-record placeholders still unfilled in the prose, code identical) | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-smoldocling-document-extraction` v2) | Default sample path, `Run all` from a fresh interpreter with an empty Hugging Face cache and no repository checkout | 1708.5 s | **PASSED** — 11/11 code cells ok (1 restart after install cell); 36 files, 560 MB staged; the metrics the build record quotes |
| 2026-09-21 | `bc77841` / `aaf8fbf0` | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-smoldocling-document-extraction` v3; image `torch 2.10.0+cu128` / `transformers 5.0.0` before the pinned install, `torch 2.14.0+cu130` / `transformers 4.57.6` after, Python 3.12.13, `cuda:0`, float32) | Default sample path, `Run all` from a fresh interpreter with an empty Hugging Face cache and no repository checkout (blob SHA-1 verified against GitHub before execution) | 1730.3 s | **PASSED** — 11/11 code cells ok (1 restart after install cell); 36 files, 560 MB staged from the Hub into a clean cache; comparison {cer: {empty: 1, constant: 0.944, frozen: 1.439, adapted: 0.782}, wer: {empty: 1, constant: 0.999, frozen: 2.491, adapted: 0.975}, cer_macro: {empty: 1, constant: 0.952, frozen: 1.392, adapted: 0.882}, exact_match: {empty: 0, constant: 0, frozen: 0, adapted: 0.014}, delta_vs_frozen: {cer: -0.657, wer: -1.516, cer_macro: -0.51, exact_match: 0.014}, hypothesis_length: {ref_chars: 6169, frozen_hyp_chars: 4839, adapted_hyp_chars: 5225, frozen_truncated: 29, adapted_truncated: 2}}; drawing / scene / page check frozen vs adapted {frozen: {word_error_rate: 0.192}, adapted: {word_error_rate: 0.709}}; reload parity {identical_lines: 8, of: 8}; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-smoldocling-document-extraction/v3/evidence/` in the workspace |
