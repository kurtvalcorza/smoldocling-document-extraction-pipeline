...
from smoldocling_document_extraction_pipeline import pipeline as pl
from smoldocling_document_extraction_pipeline import samples as sm
...
def _echo_runner(image, instruction, max_new_tokens):
    """An injected single-image runner that returns the line it was shown as DocTags (a side table by pixel digest)."""
    return {"doctags": line_doctags(_echo_runner.table.get(image_digest(image), "")) + "<end_of_utterance>", "new_tokens": 5}


def _echo_batch_runner(images, instruction, max_new_tokens):
    return [_echo_runner(image, instruction, max_new_tokens) for image in images]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: r.update(image=Image.new("RGB", (20000, 32))), "MAX_IMAGE_SIDE"),
...
def test_evaluate_scores_hypotheses_from_the_runner_and_flags_truncation():
    ...
    pipe = SmolDoclingPipeline(_echo_runner, "cpu", _batch_runner=_echo_batch_runner)
    ...
    assert result["model_revision"] == MODEL_REVISION and result["seconds"] >= 0.0 and result["instruction"] == pl.DEFAULT_INSTRUCTION
    assert result["doctags"][0] == line_doctags(records[0]["text"])  # the wrapper survives, the terminator is stripped
    with pytest.raises(ValueError, match="1..5000"):
        pipe.evaluate([])
    single = SmolDoclingPipeline(_echo_runner, "cpu")  # no batch runner: transcribe falls back to the runner per image
    ...
    with pytest.raises(ValueError, match="not one of the supported INSTRUCTIONS"):
        single.transcribe([records[0]["image"]], instruction="Transcribe this line.")
    assert line_doctags("  a   b ") == "<doctag><text><loc_0><loc_0><loc_500><loc_500>a b</text>\n</doctag>"


def test_adapt_and_artifacts_require_a_loaded_model(forbid_model_imports):
    pipe = SmolDoclingPipeline(_echo_runner, "cpu")
...
def _manifest(tmp_path, **overrides):
    ...
    manifest = {
        ...
        "tensors": ["model.text_model.layers.29.mlp.down_proj.weight", "model.text_model.norm.weight"],
        "adapter": {"best_epoch": 1, "instruction": "Convert this page to docling."},
...
def test_check_artifact_manifest_accepts_a_consistent_manifest_and_refuses_each_deviation(tmp_path):
    ...
    for name in ("model.text_model.layers.21.mlp.down_proj.weight", "model.vision_model.encoder.layers.0.x", "model.text_model.embed_tokens.weight", "model.connector.modality_projection.x", "lm_head.weight"):
    ...
    with pytest.raises(ValueError, match="adapter.instruction"):
        pl._check_artifact_manifest(_manifest(tmp_path, adapter={"best_epoch": 1}), tmp_path, "base-digest")
    with pytest.raises(ValueError, match="adapter.instruction"):
        pl._check_artifact_manifest(_manifest(tmp_path, adapter={"best_epoch": 1, "instruction": "Transcribe."}), tmp_path, "base-digest")
...
def test_trainable_names_selects_the_last_layers_and_the_norm():
    ...
    class _Model:
        def named_parameters(self):
            names = ["model.vision_model.encoder.layers.0.x", "model.connector.modality_projection.x", "model.text_model.embed_tokens.weight", "model.text_model.layers.0.x", "model.text_model.layers.21.x", "model.text_model.layers.22.x", "model.text_model.layers.29.mlp.up_proj.weight", "model.text_model.norm.weight", "lm_head.weight"]
            return [(n, _Param()) for n in names]

    assert pl._trainable_names(_Model()) == ["model.text_model.layers.22.x", "model.text_model.layers.29.mlp.up_proj.weight", "model.text_model.norm.weight"]
    assert pl._TRAINABLE_FIRST_LAYER == 22 and len(pl._TRAINABLE_PREFIXES) == 9
