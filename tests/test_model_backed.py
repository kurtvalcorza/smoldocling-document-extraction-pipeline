...
from smoldocling_document_extraction_pipeline.pipeline import _TRAINABLE_PREFIXES
...
@pytest.fixture(scope="module")
def pipe():
    return SmolDoclingPipeline.from_pretrained(weights_dir=DEFAULT_WEIGHTS_DIR)
...
def test_model_facts_batched_recognition_and_frozen_evaluation(pipe, records):
    ...
    single = [pipe.convert(r["image"], max_new_tokens=64) for r in records[:3]]
...
def test_one_epoch_adaptation_and_artifact_round_trip(pipe, records, tmp_path):
    ...
    assert result["n_trainable"] == ADAPTER_PARAMETERS and result["n_total"] == PARAMETER_COUNT and result["first_trainable_layer"] == 22 and result["instruction"] == DEFAULT_INSTRUCTION
    ...
    assert manifest["metadata"] == {"note": "test"} and manifest["adapter"]["selection"] == "lowest validation CER" and manifest["adapter"]["instruction"] == DEFAULT_INSTRUCTION
    reloaded = SmolDoclingPipeline.from_artifact(artifact, weights_dir=DEFAULT_WEIGHTS_DIR)
...
def test_load_artifact_refuses_a_tensor_set_that_differs_from_the_recorded_configuration(pipe, records, tmp_path):
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        SmolDoclingPipeline.from_artifact(fewer, weights_dir=DEFAULT_WEIGHTS_DIR)
    ...
    with pytest.raises(ValueError, match="tensor names differ"):
        SmolDoclingPipeline.from_artifact(extra, weights_dir=DEFAULT_WEIGHTS_DIR)
    ...
    with pytest.raises(ValueError, match="last 8 decoder layers"):
        SmolDoclingPipeline.from_artifact(vision, weights_dir=DEFAULT_WEIGHTS_DIR)
