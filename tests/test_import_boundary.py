...
from smoldocling_document_extraction_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
    SmolDoclingPipeline,
...
_CONFIG = json.dumps({"model_type": "idefics3", "image_token_id": 49190, "scale_factor": 4}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        SmolDoclingPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
